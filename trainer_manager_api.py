...
trainer_manager = TrainerManager("./trainers.sqlite")
    ...
    try:
        if content["type"] == "Regular Trainer":
            regular_trainer = RegularTrainer(
                ...
                content["phone_num"], content["have_partner"], content["type"])
...
@app.route("/trainermanager/trainers/all/type/<type>", methods=["GET"])
...
@app.route("/trainermanager/trainers/all/location/<location>", methods=["GET"])
def get_all_trainers_by_location(location):
    """Gets all trainers by location"""
    trainer_dict_list = []
    for curr_trainer in trainer_manager.get_all_by_location(location):
        trainer_dict_list.append(curr_trainer.to_dict())
    try:
        response = app.response_class(
            status=200,
