...
def add_trainer():
    ...
    try:
        if content['type'] == 'Regular Trainer':
            regular_trainer = RegularTrainer(content['name'], content['pokemon_team'], content['trainer_class'],
                                             content['pokecoins'], content['location'], content['movement_type'],
                                             content['phone_num'], content['have_partner'])
            response = app.response_class(response=str(
                trainer_manager.add(regular_trainer)), status=200)
        elif content['type'] == 'Gym Leader':
            gym_leader = GymLeader(content['name'], content['pokemon_team'], content['trainer_class'],
                                   content['pokecoins'], content['location'], content['badge'], content['element'], content['prize'])
            response = app.response_class(response=str(
                trainer_manager.add(gym_leader)), status=200)
        else:
    ...
    try:
        ...
        if result is not None:
            response = app.response_class(status=200, response=json.dumps(
                result.to_dict()), mimetype='application/json')
        else:
            response = app.response_class(
...
def get_all_trainers():
    ...
    return app.response_class(status=200, response=json.dumps(trainer_dict_list), mimetype='application/json')
...
def get_all_trainers_by_type(type):
    ...
    try:
        response = app.response_class(
            status=200, response=json.dumps(trainer_dict_list), mimetype='application/json')
    except Exception as e:
        response = app.response_class(response=str(e), status=400)
    return response
...
def get_stats():
    ...
    result = ['Number of Trainers: %d' % stat.get_num_trainers(),
              'Number of Gym Leaders: %d' % stat.get_num_gym_leader(),
              'Number of Regular Trainers: %d' % stat.get_num_regular_trainer(),
              'Number of Trainers With Partners: %d' % stat.get_num_trainer_have_partner(),
              ('Trainers per location: {}').format(stat.get_num_per_location())]
    return app.response_class(status=200, response=json.dumps(result), mimetype='application/json')
