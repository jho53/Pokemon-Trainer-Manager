...
class UpdateRegularTrainerPopup(tk.Frame):
    ...
    def __init__(self, parent, close_callback):
        ...
        tk.Label(self, text="Trainer Name").grid(row=2, column=1)
        self._name = tk.Entry(self)
        self._name.grid(row=2, column=2)
        tk.Label(
            self, text="Pokemon #1 - Required - Format: Pokemon, Level").grid(row=3, column=1)
        self._pokemon1 = tk.Entry(self)
        self._pokemon1.grid(row=3, column=2)
        self._pokemon1_level = tk.Entry(self)
        self._pokemon1_level.grid(row=3, column=3)
        tk.Label(
            self, text="Pokemon #2 - Optional - Format: Pokemon, Level").grid(row=4, column=1)
        self._pokemon2 = tk.Entry(self)
        self._pokemon2.grid(row=4, column=2)
        self._pokemon2_level = tk.Entry(self)
        self._pokemon2_level.grid(row=4, column=3)
        tk.Label(
            self, text="Pokemon #3 - Optional - Format: Pokemon, Level").grid(row=5, column=1)
        self._pokemon3 = tk.Entry(self)
        self._pokemon3.grid(row=5, column=2)
        self._pokemon3_level = tk.Entry(self)
        self._pokemon3_level.grid(row=5, column=3)
        tk.Label(
            self, text="Pokemon #4 - Optional - Format: Pokemon, Level").grid(row=6, column=1)
        self._pokemon4 = tk.Entry(self)
        self._pokemon4.grid(row=6, column=2)
        self._pokemon4_level = tk.Entry(self)
        self._pokemon4_level.grid(row=6, column=3)
        tk.Label(
            self, text="Pokemon #5 - Optional - Format: Pokemon, Level").grid(row=7, column=1)
        self._pokemon5 = tk.Entry(self)
        self._pokemon5.grid(row=7, column=2)
        self._pokemon5_level = tk.Entry(self)
        self._pokemon5_level.grid(row=7, column=3)
        tk.Label(
            self, text="Pokemon #6 - Optional - Format: Pokemon, Level").grid(row=8, column=1)
        self._pokemon6 = tk.Entry(self)
        self._pokemon6.grid(row=8, column=2)
        self._pokemon6_level = tk.Entry(self)
        self._pokemon6_level.grid(row=8, column=3)
        tk.Label(self, text="Trainer Class").grid(row=9, column=1)
        self._trainer_class = tk.Entry(self)
        self._trainer_class.grid(row=9, column=2)
        tk.Label(self, text="Pokecoins").grid(row=10, column=1)
        self._pokecoins = tk.Entry(self)
        self._pokecoins.grid(row=10, column=2)
        tk.Label(self, text="Location").grid(row=11, column=1)
        self._location = tk.Entry(self)
        self._location.grid(row=11, column=2)
        tk.Label(
            self, text="Movement Type - Required - Choose 1").grid(row=12, column=1)
    ...
    def _submit_cb(self):
        ...
        if re.match("^\d+$", self._trainer_id.get()) is None:
            messagebox.showerror(
                "Error", "Trainer id amount must be a valid integer")
            return

        trainer_id = self._trainer_id.get()
        ...
        response = requests.put("http://127.0.0.1:5000/trainermanager/trainers/" + trainer_id,
                                json=data, headers=headers)

        print(data)

        if response.status_code == 200:
            messagebox.showinfo("Success", "Trainer has been updated")
            self._close_cb()
        else:
            messagebox.showwarning(
                "Error", "Trainer could not be updated because of invalid data or an incorrect trainer id number")
